...
export const LaserFlow: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const outerMount = mountRef.current!;
    ...
    // Read dimensions from outerMount (always visible) to avoid Tauri GPU compositor skipping clipped layers
    const setSizeNow = () => {
      const w = outerMount.clientWidth || 1;
      const h = outerMount.clientHeight || 1;
    ...
    // Force GPU resize — resets the cache so setSizeNow always recalculates.
    // This is equivalent to what happens when DevTools opens (triggers window resize + recomposite).
    const forceResize = () => {
    ...
    const scheduleResize = () => {
      ...
      resizeRaf = requestAnimationFrame(forceResize);
    };

    setSizeNow();
    // Observe the always-visible outer wrapper for size changes
    const ro = new ResizeObserver(scheduleResize);
    ro.observe(outerMount);

    // Listen for window-level resize + focus events (this is exactly what DevTools triggers)
    window.addEventListener('resize', forceResize);
    window.addEventListener('focus', forceResize);

    // Aggressive staggered layout fallbacks — use forceResize to bypass cache
    // Tauri's webview may report 0 dimensions during early initialization
    const timer1 = setTimeout(forceResize, 50);
    const timer2 = setTimeout(forceResize, 150);
    const timer3 = setTimeout(forceResize, 300);
    const timer4 = setTimeout(forceResize, 600);
    const timer5 = setTimeout(forceResize, 1200);
    const timer6 = setTimeout(forceResize, 2500);
    ...
    // Tauri webview starts with document.hidden=true — only unpause, never pause
    // to prevent context initialization freeze in Tauri app shells
    const onVis = () => {
      if (!document.hidden) {
    ...
    // Unified tick function called by both rAF and setInterval
    const tick = () => {
      if (pausedRef.current) return;

      const t = (performance.now() - startTime) / 1000;
      const dt = Math.max(0, t - prevTime);
      if (dt < 0.001) return; // dedup: skip if called twice in same ms
      prevTime = t;
    ...
    // Primary driver: requestAnimationFrame (smooth 60fps when active)
    const animate = () => {
    ...
    // Secondary driver: setInterval fallback for Tauri webview startup
    // rAF is throttled/paused until the window receives focus in Tauri.
    // This ensures rendering happens regardless. The dedup in tick() prevents double work.
    const fallbackInterval = setInterval(tick, 32); // ~30fps fallback
  ...
  return (
    <div
      ...
      style={style}
    >
      {/* Canvas container — renders directly, no hiding */}
      <div 
